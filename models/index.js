...
class User{
    login(req, res) {
        ...
        const verifyQuery = `select userID, firstName, lastName, gender, cellphoneNumber, emailAdd, userPass, userRole, userProfile, joinDate from Users where emailAdd = '${emailAdd}';`;
...
class Product {
    ...
    addProduct(req, res) {
        ...
        db.query(addProductQuery, [req.body], (err) => {
            if(err) {
                res.status(400).json({err: 'Unable to insert a new record.'}), console.log(err);;
            }else {
